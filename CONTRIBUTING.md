...
- The `specs` directory contains markdown spec files that document the architecture, patterns, and conventions. These specs are consumed by AI assistants to generate new Flutter projects.
- The `sample` directory contains a golden reference Flutter project validated by CI. It serves as the ground truth for the validation pipeline.

### Maintaining specs

When making changes to the architecture, patterns, or conventions:

1. Update the relevant spec file(s) in `specs/`.
2. Update the `sample/` project to reflect the change.
3. Run the validation pipeline: `bash specs/validation/validate_all.sh sample`
4. Ensure all 4 validation layers pass before submitting a PR.
